...
const starts_with_some_1 = require("../helpers/starts_with_some");
...
class Instruction {
    ...
    validateAndProcessArguments(args, min, ...types) {
        if (args.length < min) {
            logger_1.Logger.error(`${colors_1.default.bgYellow(this.name)} requires at least ${colors_1.default.green(min.toString())} arguments but receives ${colors_1.default.red(args.length.toString())} instead!`, `${this.constructor.name}.validateAndProcessArguments`);
        }
    ...
    buildConditionArgument(arg) {
        if (lodash_1.default.isNil(arg))
            return "";
        let result = "", current = "", depth = 0, i = 0;
        while (i < arg.value.length) {
            const char = arg.value.charAt(i);
            const op = (0, starts_with_some_1.startsWithSome)(arg.value, i, constants_1.Operators);
    ...
    buildStringArgument(arg, input) {
        ...
        if (!value)
            return "";
        if (value in constants_1.BooleanMapping)
            return (arg.value = constants_1.BooleanMapping[value]);
        ...
        for (let i = 0; i < value.length; i++) {
            ...
            const char = value[i];
            const special = (0, starts_with_some_1.startsWithSome)(value, i, constants_1.EscapableCharacters);
    ...
    buildBooleanArgument(arg) {
        if (!arg)
            return "false";
        return constants_1.Falsys.has(arg.value) ? "false" : "true";
    }
    ;
    /**
     * Builds number arguments by processing each token in the given array of task arguments.
